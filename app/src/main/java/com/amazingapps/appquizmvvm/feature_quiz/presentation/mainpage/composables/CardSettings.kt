                    ...
                    Card(modifier.padding(8.dp)) {
                        Column(Modifier.fillMaxWidth().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                            ...
                            Row (modifier.fillMaxWidth().padding(start = 8.dp)) {
                                ...
                                Text("Efectes de soroll: ", fontFamily = FontFamily.Serif, fontSize = 18.sp, modifier = Modifier.padding(top = 13.dp))

                                val soundManager = remember { SoundManager(context) }
                                var isEnabled by remember { mutableStateOf(soundManager.isSorollActiu()) }
                                IconButton(onClick = {
                                    isEnabled = !isEnabled
                                    soundManager.soroll(isEnabled)
                                }) {
                                    Icon(
                                        imageVector = if (isEnabled) Icons.Default.Check else Icons.Default.Clear,
